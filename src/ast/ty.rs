...
impl Expr {
    ...
    fn parse_type_inner(tokens: &mut Tokens, codebase: &mut Codebase, args: ParseArgs) -> ExprId {
        ...
        if tokens.peek_bracketed(BracketType::Parentheses, codebase) &&
            tokens.peek_n(1).is_some_and(|s| matches!(s, Token::Symbol(Symbol::FatArrow, _)))
        {
            ...
            tokens.expect_symbol(Symbol::FatArrow, codebase);
            let return_ty = Expr::parse_type(tokens, codebase, args);
            return codebase.exprs.add(Expr::TyFunction { params, return_ty, span: tokens.span_from(start) });
        }
    ...
    pub(super) fn parse_type(tokens: &mut Tokens, codebase: &mut Codebase, args: ParseArgs) -> ExprId {
        let start = tokens.start();
        let inner = Expr::parse_type_inner(tokens, codebase, args);
        ...
        if tokens.peek_and_expect_symbol(Symbol::Question, codebase) {
            ...
            return codebase.exprs.add(Expr::TyOptional { inner, span: tokens.span_from(start) });
        }
        ...
        if tokens.peek_and_expect_symbol(Symbol::Scope, codebase) {
            ...
            return codebase.exprs.add(Expr::TyAccess { from: inner, item, span: tokens.span_from(start) });
        }
